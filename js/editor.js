...
function addSidebarIcons(callback) {
    var obj = {
        ...
        }, {"CIRCLE": {"X": 20, "Y": 20, "R": 10, "FILL": "#FFFFFF", "S": "black", "SW": 2}}]
    };
    var key = 'Shape';
    var el = obj[key];
...
function createPropertyDialogs(callback) {
    ...
    for (var i in properties) {
        ...
        for (var j in props) {
            ...
            switch (val) {
                case 'number':
                    parentEl.append($('<div class="searchable" title="' + props[j].name + '"><span>' + props[j].name + '</span><input type="number" title="' + j + '" min="0" value="0"/></div>'));
                    break;
                case 'text':
...
function addPolygon() {
    var polyObj = new fabric.Polygon([{x: 75, y: 0}, {x: 150, y: 60}, {x: 120, y: 135}, {x: 30, y: 135}, {
        x: 0,
        y: 60
    }], {
        fill: 'white',
