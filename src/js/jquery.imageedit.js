...
(function ($, _window) {
    ...
    $.fn.imageEdit = function (options) {
        ...
        $("#IEcontrols div").on("click", function () {
            ...
            switch (control_type) {
                ...
                case 'save':
                    ...
                    break;
                case 'overlays':
                    if ($("#IEshape_controls").hasClass("active")) {
        ...
        $("#IEshape_controls div").on("click", function () {
    ...
    var init = function (index, element, options) {
        ...
        if (src.indexOf('data:') == 0) {
            filename = 'image_' + ($.now() % 10000).toString();
        }
        ...
        $(element).data("ImageEdit", _options);
        ...
        createPropertyDialogs(function () {
            ...
            $("#prop_head span").bind("mousedown", function (event) {
                ...
                $("*").bind("mousemove", function (e) {
                    ...
                    $("#IEproperties_dialogs").css('transform', 'translate3d(' + (left - 80) + 'px,0px,0px)');
                });
    ...
    function setProperties() {
        ...
        $("#IEproperties_dialogs").css("transform", "translate3d(" + (innerWidth - 280) + "px,0,0)");
    }
    ...
    var setPoints = function () {
        if (drawPoints.length > 2) {
            var x_min = Math.min.apply([], drawPoints.map(function (p) {
                return p[0];
            }));
            var y_min = Math.min.apply([], drawPoints.map(function (p) {
                return p[1];
            }));
            drawPoints = drawPoints.map(function (p) {
                return [p[0] - x_min, p[1] - y_min];
            });
            var x_max = Math.max.apply([], drawPoints.map(function (p) {
                return p[0];
            }));
            var y_max = Math.max.apply([], drawPoints.map(function (p) {
                return p[1];
            }));
    ...
    * */
    new $AC("#_clip_", 800, 800, 0, [
        "dist/images/border/border1.png",
        "dist/images/border/border2.png"
